...
void write_byte_(ZEXTEST *a,unsigned int b,unsigned char val){
ZEXTEST *context=a;
char *db_flag=(char*)(46050);
char *trap_flag=(char*)(46060);
/**trap_flag=1;*/
//if(b>=0x7ffe)return;
//if((0x4800>b)&& (b>0x4400)){log_z(context->state.pc);}
(a)->memory[(b) & 0xffff] = (val) * ((b==0x5000) ? setflag_(val) :1);
}

void write_word_(ZEXTEST *context,unsigned int address,unsigned short x){
//if(address>=0x7ffe)return;
//if((0x4800>address)&& (address>0x4400)){log_z(0x123);log_z(address);}   
unsigned char   *memory;
    memory = ((ZEXTEST *) context)->memory;             
    memory[(address) & 0xffff] = (x);               
    memory[((address) + 1) & 0xffff] = (x) >> 8;            
        }
void setflag(int a,int x){
int* b=(int*)(46000);
//log_z(a);
//log_z(x);
if(a==0)*b=x;
}
char setflag_(char x){
      char* a= (char*)(46020);
      *a=x;
      return 1;
}

unsigned char get_flag(unsigned int x){
char* p = (char*)((x == 0x5000) ? (46030) :(46040));
return *p;
}

char read_byte_(ZEXTEST *context,unsigned int address,void *x){
  //  if(address>0x7ffe)return 0; 
    return 1;
}
char read_word_(ZEXTEST *context,unsigned int address,void *x){
   // if(address>=0x7ffe)return 0;
    return 1;
}
...
int Z80Interrupt (Z80_STATE *state, int data_on_bus, void *context)
{
        ...
        if (state->iff1) {
                ...
                //vector = state->i*256+data_on_bus;
...
int Z80Emulate (Z80_STATE *state, int number_cycles, void *context)
{
        ...
        state->pc = pc + 1;
        emulate(state, opcode, elapsed_cycles, number_cycles, context);
