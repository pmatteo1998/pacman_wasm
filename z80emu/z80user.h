...
{                                                                       \
        read_byte_((ZEXTEST *)context,address,&x) ? ((x) = (address==0x5000 | address==0x5040 ) ?  get_flag(address) : ((ZEXTEST *) context)->memory[(address) & 0xffff]):1;                      \
}
...
{                                                                       \
 ...
 (x) = read_word_((ZEXTEST *)context,address,&x) ? (memory[(address) & 0xffff] | (memory[((address) + 1) & 0xffff] << 8)) : (memory[(address) & 0xffff] | (memory[((address) + 1) & 0xffff] << 8)) ;             \
}
...
{                                                                         \
        write_byte_((ZEXTEST *)context,address,x);           \
}

#define Z80_WRITE_WORD(address, x)                                      \
{                                                                       \
	write_word_((ZEXTEST *)context,address,x);                       \
}
